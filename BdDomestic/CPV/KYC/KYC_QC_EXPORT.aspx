<%@ Page Language="C#" MasterPageFile="~/CPV/KYC/KYC_MasterPage.master" AutoEventWireup="true" CodeFile="KYC_QC_EXPORT.aspx.cs" Inherits="CPV_KYC_KYC_QC_EXPORT" Theme="SkinFile" Title="Untitled Page" %>
...
<fieldset><legend class="FormHeading">Kyc Qc Export</legend>
<br />
        ...
        <tr>
            <td class="TDWidth" colspan="3">
                <table id="tblCaseCount" runat="server" visible="false" width="100%">
                    <tr><td colspan="4" style="height: 16px">
        ...
        <asp:SqlDataSource ID="sdsSelectFormat1" runat="server" ConnectionString="<%$ ConnectionStrings:CMConnectionString %>"
          ...
          <SelectParameters>
            ...
            <asp:SessionParameter Name="ClientId" SessionField="ClientId" Type="String" /> 
                            ...
                            ProviderName="<%$ ConnectionStrings:CMConnectionString.ProviderName %>" 
                            SelectCommand="SELECT qc_details.CASE_ID, qc_details.REF_NO, ISNULL(FIRST_NAME + ' ', '') + ISNULL(MIDDLE_NAME + ' ', '') + ISNULL(LAST_NAME + ' ', '') AS APPLICANT_NAME,&#13;&#10;qc_verification_code as 'VERIFICATION_CODE', CONVERT(varchar(24), CASE_REC_DATETIME, 103) + ' ' + LTRIM(SUBSTRING(CONVERT(VARCHAR(20), CASE_REC_DATETIME, 22),10, 5) + RIGHT(CONVERT(VARCHAR(20), CASE_REC_DATETIME, 22), 3)) AS CASE_REC_DATETIME, &#13;&#10;CONVERT(varchar(24), case_send_date, 103) + ' ' + LTRIM(SUBSTRING(CONVERT(VARCHAR(20), case_send_date, 22), 10, 5) + RIGHT (CONVERT(VARCHAR(20), case_send_date, 22), 3)) AS SEND_DATETIME &#13;&#10;FROM cpv_qc_case_details qc_details left outer join cpv_kyc_case_details cc_details on qc_details.case_id=cc_details.case_id &#13;&#10; WHERE (case_send_date IS NOT NULL) AND (qc_details.CENTRE_ID = ?) AND (qc_details.CLIENT_ID = ?) AND (case_send_date >= ?) AND (case_send_date < ?) ORDER BY qc_details.CASE_ID">
                                <SelectParameters>
