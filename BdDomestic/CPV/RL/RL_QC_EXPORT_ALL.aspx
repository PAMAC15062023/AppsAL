<%@ Page Language="C#" MasterPageFile="~/CPV/RL/RL_MasterPage.master" AutoEventWireup="true" CodeFile="RL_QC_EXPORT_ALL.aspx.cs" Theme="SkinFile" Inherits="CPV_RL_RL_QC_EXPORT_ALL" Title="Untitled Page" %>
...
<fieldset><legend class="FormHeading">RL QC Export</legend>
<br />
...
<table id="tblExport" width="100%" cellpadding="0" cellspacing="0" border="0">
   ...
   <tr><td colspan="11"><hr /></td></tr>
    ...
    <table id="tblViewDetail" runat="server" cellpadding="0" cellspacing="0" border="0">
        ...
        DataKeyNames="CASE_ID" Width="100%" PageSize="100" AllowPaging="True" OnDataBound="gvOutput_DataBound" OnPageIndexChanged="gvOutput_PageIndexChanged" OnPageIndexChanging="gvOutput_PageIndexChanging"  >
            <Columns>
    ...
    <asp:Button ID="btnExport" runat="server" Text="Export" SkinID="btnexportskin" ValidationGroup="a" OnClick="btnExport_Click1"/>
                        </td>
                    </tr>
                    <tr>
                        <td align="right" colspan="4">
        ...
        <asp:SqlDataSource ID="sdsSelectFormat1" runat="server" ConnectionString="<%$ ConnectionStrings:CMConnectionString %>"
          ...
          <SelectParameters>
            ...
            <asp:SessionParameter Name="ClientId" SessionField="ClientId" Type="String" /> 
                            ...
                            ProviderName="<%$ ConnectionStrings:CMConnectionString.ProviderName %>" 
                            SelectCommand="SELECT qc_case.CASE_ID, qc_case.REF_NO, ISNULL(FIRST_NAME + ' ', '') + ISNULL(MIDDLE_NAME + ' ', '') + ISNULL(LAST_NAME + ' ', '') AS APPLICANT_NAME, qc_verification_code as VERIFICATION_CODE, CONVERT (varchar(24), CASE_RECeived_DATE, 103) + ' ' + LTRIM(SUBSTRING(CONVERT (VARCHAR(20), CASE_RECeived_DATE, 22), 10, 5) + RIGHT (CONVERT (VARCHAR(20), CASE_RECeived_DATE, 22), 3)) AS CASE_REC_DATETIME, CONVERT (varchar(24), case_SEND_DATE, 103) + ' ' + LTRIM(SUBSTRING(CONVERT (VARCHAR(20), case_SEND_DATE, 22), 10, 5) + RIGHT (CONVERT (VARCHAR(20), case_SEND_DATE, 22), 3)) AS SEND_DATETIME FROM cpv_qc_case_details qc_case left outer join CPV_RL_CASE_DETAILS rl_case on qc_case.case_id=rl_case.case_id WHERE (case_SEND_DATE IS NOT NULL) AND (qc_case.CENTRE_ID = ?) AND (qc_case.CLIENT_ID = ?) AND (case_SEND_DATE >= ?) AND (case_SEND_DATE < ?) ORDER BY qc_case.CASE_ID">
                                <SelectParameters>
